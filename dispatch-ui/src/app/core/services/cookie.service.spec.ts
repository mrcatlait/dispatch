...
describe('CookieService', () => {
  ...
  it('should get cookie value', () => {
    ...
    doc.cookie = `fs_${cookieName}=${encodeURIComponent(cookieValue)}`
  ...
  it('should set cookie', () => {
    ...
    expect(documentCookieSetterSpy).toHaveBeenCalledWith(
      [
        `fs_${cookieName}=${encodeURIComponent(cookieValue)}`,
  ...
  it('should delete cookie', () => {
    ...
    // Assert
    expect(documentCookieSetterSpy).toHaveBeenCalledWith(
      [`fs_${cookieName}=`, 'Expires=Thu, 01 Jan 1970 00:00:01 GMT', 'SameSite=Strict;'].join(';'),
    )
  })
